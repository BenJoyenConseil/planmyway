...
namespace PlanMyWay.Lib.Util
{
    public class SpreadSheetRoadmapGenerator
    {
        ...
        static public void GenerateXLS(string fileName, List<RoadMap> roadmaps, float conso, float gasCost)
        {
            ...
            for (int i = 0; i< roadmaps.Count; i++)
            {
                ...
                total += SpreadSheetRoadmapGenerator.AddTableXLS(ref doc, roadmaps[i], conso, gasCost);
        ...
        static private Total AddTableXLS(ref SpreadsheetDocument doc, RoadMap roadmap, float conso, float gasCost)
        {
            Total total = new Total();
