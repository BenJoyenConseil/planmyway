    ...
    public class MeetingManager : IMeetingManager
    {
        ...
        public void GetAllMeetingsAsync(DateTime date)
        {
            ...
            appointments.SearchCompleted += (o, e) =>
            {
                ...
                if (!e.Results.Any() || !e.Results.Where(item => item.Location != null).Any())
                {
                    Debug.WriteLine("No Apointments Found");
                    AllMeetingsRetreived(this, new AllMeetingsReceivedEventArgs() { Error = true, MessageError = "Aucun rendez-vous pour le jour sélectionné."});
                    return;
                }
